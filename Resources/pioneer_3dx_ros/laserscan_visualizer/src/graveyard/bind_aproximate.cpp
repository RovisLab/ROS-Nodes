...
{
    myfile << i << ". " << odom->pose.pose.position.x << "\t" << odom->pose.pose.position.y << "\t" << odom->pose.pose.orientation.w << "\t" << odom->twist.twist.linear.x << "\t" << odom->twist.twist.angular.z << "\t";
...
Note that mono8 and bgr8 are the two image encodings expected by most OpenCV functions.
    */
    ...
    filename_image = path + "/image_raw_laser/image." + time_string + ".png";

    string file_image = "image." + time_string + ".png";
    myfile << file_image << "\t";
    ...
    filename_laser = path + "/image_raw_laser/laser." + time_string + ".png";

    string file_laser = "laser" + time_string + ".png";
    myfile << file_laser << "\n";
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    typedef sync_policies::ApproximateTime<Image, LaserScan, Odometry> MySyncPolicy;
    // ApproximateTime takes a queue size as its constructor argument, hence MySyncPolicy(10)
    Synchronizer<MySyncPolicy> sync(MySyncPolicy(2), image_sub, laser_sub, odom_sub);
    sync.registerCallback(boost::bind(&bindCallback, _1, _2, _3));
