...
roslaunch pioneer_description pioneer_initialization.launch robot_URDF_model:="$2" pose_file:="${12}" &
pid="$pid $!"
sleep 2s
...
for i in `seq 1 ${11}`;
do
  roslaunch pioneer_nav2d localisation_launcher.launch robot_name:="pioneer$i" robot_pose_x:="$(rosparam get /pioneer$i/x)" robot_pose_y:="$(rosparam get /pioneer$i/y)" robot_pose_yaw:="$(rosparam get /pioneer$i/a)" localization_type:="$6" &
  pid="$pid $!"
  sleep 2s
done
